...
function api_dbresult_line_editor_fields($db,$teamid,$league){
	$rs = $db->query(api_sql_line_editor_fields($teamid));
	$row = $rs->fetchArray();
	foreach($row AS $id=>$r){
		if(is_numeric($id)){unset($row[$id]);}
	}
	If (strtolower($league) == "farm"){
		$row['Type'] = 901; //Add Farm - Weird Number to Easy Debug
	}else{
		$row['Type'] = 902; //Add Pro - Weird Number to Easy Debug
	}
	return $row;
}
...
function api_fields_line_editor_setup(){
	return  array(	"BlockPlayerFromPlayingLines12","BlockPlayerFromPlayingLines123","BlockPlayerFromPlayingLines12inPPPK",
					...
					"PullGoalerMinGoal","PullGoalerMinGoalEnforce","PullGoalerMinPct","PullGoalerRemoveGoaliesSecond","PullGoalerMax","FullFarmEnableGlobal","FullFarmEnableLocal");
}
...
function api_js_function_line_validator($db,$teamid,$league){
	$jsRow = api_dbresult_line_editor_fields($db,$teamid,$league);
	$f = "";
	foreach(array_keys($jsRow) AS $k){
		if(!is_numeric($k))$f .= (!is_numeric($jsRow[$k])) ? strtolower($jsRow[$k]) . "," : $jsRow[$k] .",";
	}
		...
		<div class="pagewrapper pagewrapperrostereditor"><?php 
			...
			if(isset($_POST["sbtRoster"])){
				...
				if(count($arrSort) > 0){
					...
					//Update The WebClientIP
					$sql .= "UPDATE TeamFarmInfo SET FullFarm = '". (($_POST['FullFarmEnableLocal'] == "true") ? 'True' : 'False') ."'";
					$sql .= ", WebClientModify = 'True' ";
					$sql .= ", WebClientIP = '" . $_SERVER['REMOTE_ADDR'] . "' ";	
					$sql .= "WHERE Number = " . $teamid . ";";	
					$db->exec($sql);		
...
function api_pageinfo_editor_lines($db,$teamid=0,$league=false,$showHeader=true,$useServerURIInTabLink=false){	
	...
	if($teamid > 0){
		...
		$dbfields = api_dbresult_line_editor_fields($db,$teamid,$league);
		$cpfields = "";
...
function api_sql_line_editor_fields($teamid){
