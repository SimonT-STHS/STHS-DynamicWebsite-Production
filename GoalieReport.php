...
If ($Goalie == 0){
	...
	If ($Result['count'] == 1){
		...
		$Query = "SELECT * FROM NextYearFreeAgent WHERE Number = " . $Goalie . " and PlayerType = \"False\"";
		$GoalerNextYearContract = $db->querySingle($Query,true);
		...
		If ($GoalieInfo['Team'] > 0){
			...
			$Query = "SELECT TeamThemeID FROM TeamProInfo WHERE Number = " . $GoalieInfo['Team'];
		...
		If (file_exists($CareerStatDatabaseFile) == true){ /* CareerStat */
			...
			If ($CareerDBFormatV2CheckCheck['CountName'] == 1){
				...
				$GoalieProCareerPlayoff = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatProHistoryAllSeasonPerYear' => '', 'UniqueID' => $GoalieInfo['UniqueID'], 'Playoff' => ''));
				...
				$GoalieProCareerSumSeasonOnly = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatProHistoryAllSeasonMerge' => '', 'UniqueID' => $GoalieInfo['UniqueID']));
				...
				$GoalieProCareerSumPlayoffOnly = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatProHistoryAllSeasonMerge' => '', 'UniqueID' => $GoalieInfo['UniqueID'], 'Playoff' => ''));
				...
				$GoalieFarmCareerSeason = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatFarmHistoryAllSeasonPerYear' => '', 'UniqueID' => $GoalieInfo['UniqueID']));
				...
				$GoalieFarmCareerPlayoff = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatFarmHistoryAllSeasonPerYear' => '', 'UniqueID' => $GoalieInfo['UniqueID'], 'Playoff' => ''));
				...
				$GoalieFarmCareerSumSeasonOnly = APIPost($LeagueOutputOption['WebsiteURL'],array('GoalerStatFarmHistoryAllSeasonMerge' => '', 'UniqueID' => $GoalieInfo['UniqueID']));
...
If ($TeamInfo['TeamThemeID'] > 0){
	...
	echo ".STHSPHPPlayerStat_Table th, .tablesorter thead td {color:var(--TeamNameColor_TextColor_" . $TeamInfo['TeamThemeID'] . ")  !important; background-color: var(--TeamNameColor_Background_" . $TeamInfo['TeamThemeID'] . ")  !important;}\n";
}
echo "</style>";
?>
</head><body>
...
If($GoalieInfo <> Null AND $LeagueOutputOption <> Null){
	if ($GoalieInfo['Retire'] == 'False'){
		if (empty($TeamPlayers) == false){
			echo "<div id=\"cssmenu\" style=\"display:inline-block;z-index:5;\"><ul class=\"STHSCursorUL1\"><li class=\"STHSCursorLI1\">&#9660;<ul class=\"STHSCursorUL2\">";
			while ($Row = $TeamPlayers ->fetchArray()) { 
				if ($Row['PosG']== "True"){
		...
		echo "<br><br>" . $GoalieInfo['TeamName'] . "</td>";
	}else{
...
<tr>
	<th colspan="18"><?php echo $PlayersLang['Rank'];?></th>
</tr><tr>
<?php
If($GoalieInfo != Null){
	$RatingArray = array("SK","DU","EN","SZ","AG","RB","SC","HS","RT","PH","PS","EX","LD","PO","MO","Overall"); 
	foreach ($RatingArray as $Rating) {
		$QueryForPlayerRatingRank = "SELECT ROW_NUMBER() OVER(ORDER BY (GoalerInfo." . $Rating. ") DESC, GoalerInfo.Overall DESC) AS PlayerRatingRank, GoalerInfo.Number, GoalerInfo." . $Rating . ", GoalerInfo.Overall FROM GoalerInfo";
		$PlayerRatingRank = $db->query($QueryForPlayerRatingRank);
		if (empty($PlayerRatingRank) == false){while ($Row = $PlayerRatingRank ->fetchArray()) {
		if($GoalieInfo['Number'] == $Row['Number']){
		echo "<td>" . $Row['PlayerRatingRank'];
		if ($Row['PlayerRatingRank'] == 1) { echo "er". "</td>";; }else{echo "e". "</td>\n";;}}}}	
	}
...
<?php
If($GoalieInfo != Null){
	echo "<td>"; If($GoalieInfo['SalaryCap2'] > 0){echo number_format($GoalieInfo['SalaryCap2'],0) . "$";}else{echo "-";}echo "</td>";
	echo "<td>"; If($GoalieInfo['SalaryCap3'] > 0){echo number_format($GoalieInfo['SalaryCap3'],0) . "$";}else{echo "-";}echo "</td>";
	echo "<td>"; If($GoalieInfo['SalaryCap4'] > 0){echo number_format($GoalieInfo['SalaryCap4'],0) . "$";}else{echo "-";}echo "</td>";
	echo "<td>"; If($GoalieInfo['SalaryCap5'] > 0){echo number_format($GoalieInfo['SalaryCap5'],0) . "$";}else{echo "-";}echo "</td>";
	echo "<td>"; If($GoalieInfo['SalaryCap6'] > 0){echo number_format($GoalieInfo['SalaryCap6'],0) . "$";}else{echo "-";}echo "</td>";
}?>
</tr>
</table>
<div class="STHSBlankDiv"></div>
...
<?php
If ($GoalerNextYearContract != Null){
	...
	echo "</tr><tr>";
	echo "<td>" . $GoalerNextYearContract['Contract'] . "</td>";
	echo "<td>" . number_format($GoalerNextYearContract['Salary'],0) . "$";echo "</td>"; 
	echo "<td>"; if($$GoalerNextYearContract['CanPlayPro']== "True"){ echo "Yes"; }else{echo "No";}echo "</td>";
	echo "<td>"; if($$GoalerNextYearContract['CanPlayFarm']== "True"){ echo "Yes"; }else{echo "No";}echo "</td>";	
	echo "<td>"; if($$GoalerNextYearContract['ProSalaryinFarm']== "True"){ echo "Yes"; }else{echo "No";}echo "</td>";
