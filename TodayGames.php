...
If (file_exists($DatabaseFile) == false){
	...
	if($Type == 1){
		...
		$QueryLastestScore = "SELECT *,'Pro' as Type FROM SchedulePro WHERE Day >= " . ($LeagueGeneral['ScheduleNextDay'] - $LeagueGeneral['DefaultSimulationPerDay']) . " AND PLAY = 'True' ORDER BY GameNumber ";
		...
		$QueryLastestScore = "SELECT *,'Farm' as Type FROM ScheduleFarm WHERE Day >= " . ($LeagueGeneral['ScheduleNextDay'] - $LeagueGeneral['DefaultSimulationPerDay']) . " AND PLAY = 'True' ORDER BY GameNumber ";
		...
		$QueryLastestScore = "SELECT *,'Pro' as Type FROM SchedulePro WHERE Day >= " . ($LeagueGeneral['ScheduleNextDay'] - $LeagueGeneral['DefaultSimulationPerDay']) . " AND PLAY = 'True' UNION SELECT *,'Farm' as Type FROM ScheduleFarm WHERE Day = " . ($LeagueGeneral['ScheduleNextDay'] - $LeagueGeneral['DefaultSimulationPerDay']) . " AND PLAY = 'True' ORDER BY TYPE DESC, GAMENUMBER";
	...
	$LatestScore = $db->query($QueryLastestScore);
...
Function PrintGamesLastestScore($Row, $ScheduleLang, $LeagueOutputOption, $ImagesCDNPath){
	echo "<div class=\"STHSTodayGame_GameOverall\"><table class=\"STHSTodayGame_GameTitle\"><tr><td class=\"STHSTodayGame_GameNumber\"><h3>";
	If ($Row['Type'] == "Pro"){
		echo $ScheduleLang['ProGames'] . $Row['GameNumber'];
	}elseif ($Row['Type'] == "Farm"){
		echo $ScheduleLang['FarmGames'] . $Row['GameNumber'];
	}else{
	...
	echo $Row['VisitorTeamName'] ."</h3></td>";
	echo "<td class=\"STHSTodayGame_TeamScore\"><h3>";
	If ($Row['VisitorScore'] > $Row['HomeScore']){echo "<span style=\"color:red;font-weight:bold;\">" . $Row['VisitorScore'] ."</span>";}else{echo $Row['VisitorScore'];}
	echo "</h3></td></tr><tr>";
	echo "<td class=\"STHSTodayGame_TeamName\"><h3>";
	If ($Row['HomeTeamThemeID'] > 0){echo "<img src=\"" . $ImagesCDNPath . "/images/" . $Row['HomeTeamThemeID'] .".png\" alt=\"\" class=\"STHSPHPTodayGameTeamImage\">";}	
	echo $Row['HomeTeamName'] ."</h3></td>";
	echo "<td class=\"STHSTodayGame_TeamScore\"><h3>";
	If ($Row['HomeScore'] > $Row['VisitorScore']){echo "<span style=\"color:red;font-weight:bold;\">" . $Row['HomeScore'] ."</span>";}else{echo $Row['HomeScore'];}
	echo "</h3></td></tr>";
	echo "</tr></table></div>\n";
}
Function PrintGames($Row, $ScheduleLang, $LeagueOutputOption, $ImagesCDNPath){
	...
	echo "<tr><td style=\"text-align:right;width:75px;\"><img src=\"" . $ImagesCDNPath . "/images/Star1.png\" alt=\"Star1\" class=\"STHSIndex_3StarImage\"></td><td style=\"text-align:left;\">" . $Row['Star1'] . "</td></tr>";
	echo "<tr><td style=\"text-align:right;width:75px;\"><img src=\"" . $ImagesCDNPath . "/images/Star2.png\" alt=\"Star2\" class=\"STHSIndex_3StarImage\"></td><td style=\"text-align:left;\">" . $Row['Star2'] . "</td></tr>";
	echo "<tr><td style=\"text-align:right;width:75px;\"><img src=\"" . $ImagesCDNPath . "/images/Star3.png\" alt=\"Star3\" class=\"STHSIndex_3StarImage\"></td><td style=\"text-align:left;\">" . $Row['Star3'] . "</td></tr></table>";	
...
.TodayGameDiv {
 ...
 <?php If ($CookieTeamWebsiteThemeID == 2){echo ".STHSIndex_3StarImage {filter: invert(100%);}\n";}?>
</style>
</head><body>
...
If ($LoopCount == 0){
	if (empty($LatestScore) == false){while ($Row = $LatestScore ->fetchArray()) {
		...
		PrintGamesLastestScore($Row, $ScheduleLang,$LeagueOutputOption,$ImagesCDNPath);
	}}
}
