...
If (file_exists($DatabaseFile) == false){
	...
	If($MaximumResult == 0){$Title = "All ";}else{$Title = "Top " .$MaximumResult . " ";}
	If ($ProOrNot == TRUE){
		/* Pro Query */
		...
		if($Team > 0){
			...
			$Title = $Title . $TeamName['Name'] . " Pro Goalies Stat";
		}else{
			$Title = $Title . " Pro Goalies Stat";
		}
		...
		if($Team > 0){
			...
			$TeamName = $db->querySingle($QueryTeam,true);	
			$Title = $Title . $TeamName['Name'] . " Farm Goalies Stat";
		}else{
			$Title = $Title . " Farm Goalies Stat";
		}		
