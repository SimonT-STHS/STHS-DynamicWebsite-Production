...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.G) AS SumOfG, Sum(Player" . $TypeText . "StatCareer.A) AS SumOfA FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfG + MainTable.SumOfA + IfNull(Player" . $TypeText . "Stat.G,0) + IfNull(Player" . $TypeText . "Stat.A,0)) DESC, (MainTable.SumOfGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.G) AS SumOfG FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfG + IfNull(Player" . $TypeText . "Stat.G,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.A) AS SumOfA FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfA + IfNull(Player" . $TypeText . "Stat.A,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.PlusMinus) AS SumOfPlusMinus FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfPlusMinus + IfNull(Player" . $TypeText . "Stat.PlusMinus,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.Pim) AS SumOfPim FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfPim + IfNull(Player" . $TypeText . "Stat.Pim,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.Hits) AS SumOfHits FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfHits + IfNull(Player" . $TypeText . "Stat.Hits,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.PPG) AS SumOfPPG FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfPPG + IfNull(Player" . $TypeText . "Stat.PPG,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.PKG) AS SumOfPKG FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfPKG + IfNull(Player" . $TypeText . "Stat.PKG,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.GW) AS SumOfGW FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfGW + IfNull(Player" . $TypeText . "Stat.GW,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.SecondPlay) AS SumOfSecondPlay FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfSecondPlay + IfNull(Player" . $TypeText . "Stat.SecondPlay,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.HatTrick) AS SumOfHatTrick FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfHatTrick + IfNull(Player" . $TypeText . "Stat.HatTrick,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
<?php
$Query = "SELECT MainTable.*, Player" . $TypeText . "Stat.*, PlayerInfo.NHLID, PlayerInfo.Country, PlayerInfo.TeamName FROM (SELECT Name AS SumOfName, UniqueID, Sum(Player" . $TypeText . "StatCareer.GP) AS SumOfGP, Sum(Player" . $TypeText . "StatCareer.HitsTook) AS SumOfHitsTook FROM Player" . $TypeText . "StatCareer WHERE Playoff = \"" . $Playoff . "\" GROUP BY Player" . $TypeText . "StatCareer.Name) AS MainTable LEFT JOIN Player" . $TypeText . "Stat ON MainTable.SumOfName = Player" . $TypeText . "Stat.Name LEFT JOIN PlayerInfo ON MainTable.SumOfName = PlayerInfo.Name ORDER BY (MainTable.SumOfHitsTook + IfNull(Player" . $TypeText . "Stat.HitsTook,0)) DESC, (MainTable.SumofGP + IfNull(Player" . $TypeText . "Stat.GP,0)) ASC";
If ($MaximumResult > 0){$Query = $Query . " LIMIT " . $MaximumResult;}
If ($Title == $DatabaseNotFound OR $Title == $CareeratabaseNotFound){$PlayerStat = Null;}else{$PlayerStat = $CareerStatdb->query($Query);}
$LoopCount = (integer)0;
if (empty($PlayerStat) == false){while ($Row = $PlayerStat ->fetchArray()) {
...
if (empty($GoalerStat) == false){while ($Row = $GoalerStat ->fetchArray()) {
	...
	echo "<td>" . ($Row['SumOfGP'] + $Row['GP']) . "</td><td>" . ($Row['Shootout'] + $Row['SumOfShootout']) .  "</td></tr>\n";
}}
If ($LoopCount > 1){
	echo "</table></td><td class=\"STHSWP2\"></td>";
}else{
