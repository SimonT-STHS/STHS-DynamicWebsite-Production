...
If ($Team == 0){
	$Title = "League Pro Schedule";
	
	$MainQuery = "SELECT * FROM SchedulePro ORDER BY GameNumber";
}else{
	$Query = "SELECT Name FROM TeamProInfo WHERE Number = " . $Team ;
	$TeamName = $db->querySingle($Query);
	$Title = $TeamName . " Pro Schedule";
	
	$MainQuery = "SELECT * FROM SchedulePro WHERE (VisitorTeam = " . $Team . " OR HomeTeam = " . $Team . ") ORDER BY GameNumber";
}
$ProSchedule = $db->query($MainQuery);
...
<?php
while ($row = $ProSchedule ->fetchArray()) {
?>
    <tr>
